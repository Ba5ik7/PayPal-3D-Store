package  com.wesley.compontents.ComboBox
{
	...
	public class ComboBox extends Sprite{
		...
		public function ComboBox(_stageW : Stage, _title:String, _arr:Array) {
			_stage = _stageW;
			if(_arr[0] is ObjImg) {
				var i : int;
				for each (var obj : ObjImg in _arr) {
					item = new ComboBoxListItem(obj, true);
					item.y = 0;
					item.name = "item" + i;
			...
			scroller = new ComboBoxScroller(_stageW, 300, 101, arrListItems);
			scroller.y = -scroller.height;
			
			buildMasker();
			createTitleTxt(_title);
			createSelectedItemLabel(_title);
		}
		...
		private function itemClick(event:MouseEvent):void  {
			...
			if (event.target.objImg) {
				_evt = new BtnEvent("btnColorCLick");
				_evt.data = event.target;
